...
```bash
# Tạo dữ liệu cho ngày mặc định (2025-08-04) → data/synthetic/20250804_sensor_data.csv
python generate_excavator_data.py

# Tạo dữ liệu cho ngày cụ thể → data/synthetic/20250815_sensor_data.csv
...
df.to_csv('data/synthetic/basic_data.csv', index=False)
...
df.to_csv('data/synthetic/custom_hours.csv', index=False)
```
...
Các tùy chọn:
  ...
  --output OUTPUT       Tên file xuất (mặc định: data/synthetic/YYYYMMDD_sensor_data.csv)
...
Days:       1
Seed:       42
Output:     data/synthetic/20250804_sensor_data.csv
Morning:   07:00 - 09:45
Afternoon: 13:45 - 17:00

⏳ Generating data...
💾 Saving to data/synthetic/20250804_sensor_data.csv...
...
⏰ Working Hours (VN time):
  Morning:   07:00 - 09:45
  Afternoon: 13:45 - 17:00
📁 File saved: data/synthetic/20250804_sensor_data.csv
```
...
├── data/
│   └── synthetic/                 # Thư mục chứa dữ liệu tổng hợp được tạo ra
│       └── *_sensor_data.csv      # Các file dữ liệu IMU tổng hợp
└── ...
```
...
df = pd.read_csv('data/synthetic/20250804_sensor_data.csv')
...
- ✅ **Thư mục xuất dữ liệu**: Lưu vào `data/synthetic/` thay vì thư mục hiện tại
