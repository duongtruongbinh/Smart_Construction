...
class RuleBasedPredictor:
    ...
    def predict_score(self, df1s: pd.DataFrame) -> pd.Series:
        ...
        return pd.Series(
            p.w_acc * z_acc + p.w_gyro * z_gyro, index=df1s.index, name="activity_score"
        )

    @staticmethod
    def _hysteresis(
        scores: np.ndarray, on_th: float, off_th: float, fs: float, debounce_sec: float
    ) -> list[int]:
    ...
    def predict(self, df1s: pd.DataFrame) -> pd.Series:
        ...
        p = self.params
        states = self._hysteresis(
            score.values, p.on_th, p.off_th, p.fs_ds, p.debounce_sec
        )
