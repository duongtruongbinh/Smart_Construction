...
def prep_time(
    df: pd.DataFrame, utc_col: str = "Timestamp", tz: str = "Asia/Ho_Chi_Minh"
) -> pd.DataFrame:
    df = df.copy()
...
def clean_and_resample(
    df: pd.DataFrame, resample_rule: str = "1s", cols_keep: list[str] | None = None
) -> pd.DataFrame:
